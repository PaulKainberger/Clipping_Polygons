...
/**
 * Represents a polygon defined by vertices which are two dimensional double
 * points. The first and the last point is connected by an edge. Two adjacent
 * vertices are different. Two points are considered equal if they only differ
 * by eps=0.0000001. This avoids double points arising from rounding during
 * numerical calculations. However, it is nevertheless suggested to make sure
 * that vertices are not that close because methods like contains() might give
 * wrong results then.
 * 
 * @author Philipp
 * @version 0.1
...
public class Polygon {
	...
	/**
	 * Adds a vertex to the end of the polygon. The first vertex has the index 0. If
	 * the vertex violates the condition that two adjacent vertices are not the same
	...
	/**
	 * Adds a vertex at a given index of the polygon. The first vertex has the index
	 * 0. If the vertex violates the condition that two adjacent vertices are not
	 * the same then it is not added.
	 * 
	 * @param vertex The vertex which is added to the polygon.
	 * @param index  The index at which the vertex will be after the insertion, i.e.
	 *               the vertex will be added before the vertex with the given
	 *               index.
	 * @return True if the vertex has been added and false otherwise.
	 * @throws If the given index is out of range (i.e. either index < 0 or index >
	 *            numberVertices an IndexOutOfBoundsException will be thrown.
	 */
	 ...
	 * @param y     The y coordinate of the vertex.
	 * @param index The index at which the vertex will be after the insertion, i.e.
	 *              the vertex will be added before the vertex with the given index.
	 * @return True if the vertex has been added and false otherwise.
	 * @throws If the given index is out of range (i.e. either index < 0 or index >
	 *            numberVertices an IndexOutOfBoundsException will be thrown.
	 */
	...
	/**
	 * Getter for the number of vertices in the polygon. The first vertex has the
	 * index 0.
	 * 
	...
	/**
	 * Checks if the polygon is empty, i.e. there are no vertices in the polygon.
	 * 
	...
	/**
	 * Getter for vertex at given index. Indices start with 0, so the last vertex is
	 * at index getNumberVertices() - 1
	 * 
	...
	/**
	 * Computes the bounds of the polygon. The top-left corner has the smallest x
	 * and y coordinates. If the polygon is empty null will be returned. If it only
	 * contains one vertex then a rectangle with 0 size at that point will be
	 * returned.
	 * 
	...
	/**
	 * Used to get a canonical vertex of the polygon, namely the vertex with the
	 * smallest x coordinate, and if that is not unique, the one with the smallest y
	...
	/**
	 * Checks if the polygon is oriented clockwise. Implements the method suggested
	 * in: https://stackoverflow.com/a/1165943/7698457 Edge cases (e.g. a single
	 * point or empty polygon) are considered as not clockwise. True if the vertex
	 * is an entry vertex, false if it is an exit vertex.
	 * 
	...
	/**
	 * Checks if given point is inside the polygon. We use the even-odd (winding
	 * number) rule for determining this for self-intersecting polygons, cf.
	 * https://en.wikipedia.org/wiki/Even%E2%80%93odd_rule A point on the polygon
	 * (i.e. a vertex or on an edge) itself is considered inside.
	 * 
	...
	public boolean contains(Point2D.Double point) {
		...
		for (int i = 0; i < vertices.size(); i++) {
			...
			if ((vi.getY() > point.getY()) != (vj.getY() > point.getY()) && (point.getX() < vi.getX()
					+ (vj.getX() - vi.getX()) * (point.getY() - vi.getY()) / (vj.getY() - vi.getY()))) {
	...
	/**
	 * Checks if a given polygon is inside the polygon. We calculate all
	 * intersection points of the two polygons and consider their location. We
	 * consider a polygon p as contained in another one, if all points of p lie
	 * inside or on the edge of the other polygon i.e. a polygon contains itself.
	 * 
	 ...
	 */
	static boolean isBetween(Point2D.Double pointStart, Point2D.Double pointEnd, Point2D.Double point) {
	 ...
	 * @param point2Start      Start point of second line.
	 * @param point2End        End point of second line.
	 * @param secondLineProper If true the second line will be assumed to be an
	 *                         entire line (not only a segment) defined by the two
	 *                         given points, and if false it will be assumed to be
	 *                         really a line segment.
	 * @return An object representing the line intersection.
	 */
	static LineIntersection intersectLines(Point2D.Double point1Start, Point2D.Double point1End,
			Point2D.Double point2Start, Point2D.Double point2End, boolean secondLineProper) {
		...
		if (Math.abs(denom) > eps) {
			...
			if ((point1Start.equals(point2Start) && !isBetween(point2Start, point2End, point1End)
					&& !isBetween(point1Start, point1End, point2End))
					|| (point1Start.equals(point2End) && !isBetween(point2Start, point2End, point1End)
							&& !isBetween(point1Start, point1End, point2Start))) {
			...
			} else if ((point1End.equals(point2Start) && !isBetween(point2Start, point2End, point1Start)
					&& !isBetween(point1Start, point1End, point2End))
					|| (point1End.equals(point2End) && !isBetween(point2Start, point2End, point1Start)
							&& !isBetween(point1Start, point1End, point2Start))) {
	...
	/**
	 * Checks if a point is to the left of a line defined by two points, going
	 * through edge1 and edge2 (coming from the direction of edge1).
	 * 
	...
	public static boolean inside(Point2D.Double point, Point2D.Double edge1, Point2D.Double edge2) {
		return (point.getX() - edge2.getX()) * (edge1.getY() - edge2.getY()) > (point.getY() - edge2.getY())
				* (edge1.getX() - edge2.getX());
	}

	/**
	 * Checks if two points are equal up to the epsilon of the class, i.e. their x
	 * and y coordinate only differ by the epsilon in both directions.
	 * 
	 ...
	 * 
	 * @param points Set of points for which we want to check if it contains other
	 *               point.
	 * @param point  Point for which we want to check if it is in set.
	 * @return True if the point is in the set of points with coordinates considered
	 *         up to an epsilon and false otherwise.
	 */
	...
	/**
	 * Removes points which are in the set twice, up to an epsilon. The points of
	 * the set are not cloned, so if they are used after this method, the set points
	 * should be cloned before.
	 * 
	...
	/**
	 * Computes the points of intersections of this polygon with a second given
	 * polygon. If two edges agree on a line segment we add both end points to the
	 * intersection points. We use the naive approach of intersecting all edges of
	 * one polygon with all edges of the other polygon.
	 * 
	 * @param polygon The second polygon we want to compute the intersection points
	 *                with.
		...
		if (getNumberVertices() == 1) {
			...
			if (polygon.getNumberVertices() == 1) {
				...
				for (int i = 0; i < numVert; i++) {
					LineIntersection point = intersectLines(singlePoint, singlePoint, polygon.getVertex(i),
							polygon.getVertex((i + 1) % numVert));
	...
	/**
	 * Checks if the polygon is self-intersecting, i.e. if two edges intersect in
	 * different points than the vertices of the polygon. We use a slow naive
	 * approach and check all possible combinations. If two edges agree in some
	 * section we also call the polygon self-intersecting. This is e.g. the case for
	 * polygons with only two vertices.
	 * 
	...
	public boolean isSelfIntersecting() {
		...
		for (int i = 0; i < numVertices; i++) {
			for (int j = i + 1; j < numVertices; j++) {
				LineIntersection point = intersectLines(getVertex(i), getVertex((i + 1) % numVertices), getVertex(j),
						getVertex((j + 1) % numVertices));
				// there is an intersection point and the two edges are
				// not adjacent (otherwise there intersection point is just the vertex)
				if (point.getType() == LineIntersection.IntersectionType.POINT && i + 1 != j
						&& Math.floorMod(i - 1, numVertices) != j) {
					return true;
				}
			}
		}
	...
	/**
	 * Checks if the polygon is convex. The algorithm assumes that the polygon is
	 * not self-intersecting, so one should use isSelfIntersecting() prior to using
	 * isConvex(). Furthermore the algorithm assumes that three adjacent vertices
	 * are not collinear, i.e. that an interior angle cannot be 2 pi. Degenerate
	 * cases with less than 3 vertices are considered not convex. We use the method
	 * suggested in: https://stackoverflow.com/a/45372025/7698457
	 * 
	...
	/**
	 * Checks if two objects are the same. Two polygons are considered the same if
	 * they contain the exact (not up to epsilon) same vertices in the same order.
	 * 
	 * @param polygon The second polygon which we want to check if it is equal to
	 *                this.
	...
	public boolean equals(Object polygon) {
		if (this == polygon) {
			...
			if (secPolygon.getNumberVertices() != getNumberVertices()) {
				...
				for (int i = 0; i < getNumberVertices(); i++) {
					if (!(secPolygon.getVertex((leastIndexSec + i) % numVertices)
							.equals(getVertex((leastIndexThis + i) % numVertices)))) {
						return false;
					}
				}
				return true;
	...
	/**
	 * Returns the hash code of the polygon. The code can differ if polygons are
	 * equal up to an epsilon, but if they are exactly equal it will be the same.
	 * 
	...
	/**
	 * Removes all vertices from the polygon, i.e. sets the polygon to the empty
	 * polygon.
	 */
	public void clear() {
		vertices.clear();
	...
	public void drawIncomplete(Graphics2D g, int screenWidth, int screenHeight, Color c, double scalingFactor) {
		...
		for (int i = 0; i < getNumberVertices() - 1; i++) {
			...
			y2 = (int) (getVertex(i + 1).getY() * scalingFactor) + screenHeight / 2;
	...
	public void draw(Graphics2D g, int screenWidth, int screenHeight, Color c, float alpha, double scalingFactor) {
		...
		g.fillPolygon(getScaledXintValues(scalingFactor, screenWidth), getScaledYintValues(scalingFactor, screenHeight),
				getNumberVertices());
	}

	public void writeToFile(FileWriter writer) throws IOException {
		for (Point2D.Double v : vertices) {
			writer.write(Double.toString(v.x));
			writer.write(",");
			writer.write(Double.toString(v.y));
			writer.write('\n');
		}
	}
}
